...
export default function DayPlan_drive({ navigation, route }) {
    let displayPlan = [];
    let routeArray = [];
            ...
            axios
                ...
                .then((response) => {
                    ...
                    for (let i = 0; i < response.data.length; i++) {
                        ...
                        console.log("Compare driverID:", response.data[i].routeUserID, "local driverID:", route.params.userID)
                        if (tempDate.getUTCDate() === selectedDateObj.getUTCDate() && tempDate.getMonth() === selectedDateObj.getMonth()
                            && tempDate.getFullYear() === selectedDateObj.getFullYear() && response.data[i].routeUserID === route.params.userID) {
                            routeArray.push({
