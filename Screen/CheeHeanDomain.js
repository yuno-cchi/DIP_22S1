  ...
  for (let index = 0; index < markerArray.length; index++) {
    if (markerArray[index].key === key) {
      markerArray[index] = {
        ...
        markerColor:
          markerArray[index].markerColor === color1 ? color2 : color1,
      };
    }
  }
...
export default function CheeHeanDomain() {
    ...
    axios
      ...
      .then((data) => {
        ...
        for (var x = 0; x < waypoints.latitude.length; x++) {
          ...
          xArray.push({
            key: x,
            userID: dataSon[1]._id,
            latitude: parseFloat(waypoints.latitude[x]),
            longitude: parseFloat(waypoints.longitude[x]),
            markerColor: color.red,
            travelDate: "2022-09-14",
          });
        }
  ...
  if (loading) {
    ...
    return (
      <View
        style={styles.container && <Text>Loading, Please Wait...</Text>}
      ></View>
    );
  }
          ...
          ? loadedRoute.map(({ key, latitude, longitude, markerColor }) => (
              <Marker
                ...
                onPress={() => {
                  ...
                  const newMarkerArray = setMarkerColor(
                    loadedRoute,
                    key,
                    color.red,
                    color.green
